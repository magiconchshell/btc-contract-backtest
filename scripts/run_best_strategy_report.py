...
def main():
    ...
    engine_base = FuturesBacktestEngine(contract, account, RiskConfig(), timeframe=timeframe, execution=execution, live_risk=live_risk)
    df = engine_base.fetch_historical_data(start, end)

    candidates = [
        {
            "name": "short_overlay_switcher_best",
            "strategy": build_strategy("short_overlay_switcher", {
                "fast_ema": 50,
                "slow_ema": 200,
                "crash_lookback": 16,
    ...
    for item in candidates:
        engine = FuturesBacktestEngine(contract, account, item["risk"], timeframe=timeframe, execution=execution, live_risk=live_risk)
        ...
        (raw_dir / f"{item['name']}_metrics.json").write_text(json.dumps(metrics, indent=2, ensure_ascii=False, default=str), encoding="utf-8")
    ...
    print(json.dumps({"report": str(out_html), "dir": str(raw_dir)}, ensure_ascii=False))
