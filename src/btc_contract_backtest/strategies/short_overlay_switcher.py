...
from .btc_bias import ExtremeDowntrendShortStrategy


class ShortOverlaySwitcherStrategy(BaseStrategy):
    def __init__(
        self,
        fast_ema: int = 50,
        slow_ema: int = 200,
        ...
        self.allow_bull_long = allow_bull_long
    ...
    def generate_signals(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.allow_bull_long:
            df.loc[bull_regime, "signal"] = 1
        df["module_source"] = "flat"
        df.loc[df["signal"] == -1, "module_source"] = "short_module"
        df.loc[df["signal"] == 1, "module_source"] = "bull_long_passthrough"
        return df
