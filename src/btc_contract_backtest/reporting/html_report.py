  ...
  <div class="wrap">
    <div class="hero">
      <div>
        ...
        <div class="small">期間：2025-01-01 至今｜本金：100 USD｜市場：Perpetual Futures｜可做多做空｜槓桿依策略設定</div>
      </div>
...
}], {{title:'總報酬 (%)', paper_bgcolor:'rgba(0,0,0,0)', plot_bgcolor:'rgba(0,0,0,0)', font:{{color:'#e8ecf3'}}}});
...
Plotly.newPlot('drawdown-chart', [{
  ...
  marker:{{color:'#f87171'}}
}], {{title:'最大回撤 (%)', paper_bgcolor:'rgba(0,0,0,0)', plot_bgcolor:'rgba(0,0,0,0)', font:{{color:'#e8ecf3'}}}});

Plotly.newPlot('trades-chart', [{
  type:'bar', x: strategies.map(s=>s.name), y: strategies.map(s=>s.metrics.total_trades), marker:{{color:'#a78bfa'}}
}], {{title:'交易數', paper_bgcolor:'rgba(0,0,0,0)', plot_bgcolor:'rgba(0,0,0,0)', font:{{color:'#e8ecf3'}}}});

Plotly.newPlot('winrate-chart', [{
  type:'bar', x: strategies.map(s=>s.name), y: strategies.map(s=>s.metrics.win_rate), marker:{{color:'#22d3ee'}}
}], {{title:'勝率 (%)', paper_bgcolor:'rgba(0,0,0,0)', plot_bgcolor:'rgba(0,0,0,0)', font:{{color:'#e8ecf3'}}}});
