from btc_contract_backtest.strategies.indicators import MACDCrossStrategy, RSIReversalStrategy, SMACrossStrategy
import pandas as pd


def sample_df():
    prices = [100 + i for i in range(60)]
    return pd.DataFrame({"close": prices, "open": prices, "high": prices, "low": prices, "volume": [1]*60})
