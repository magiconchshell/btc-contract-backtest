...
from btc_contract_backtest.live.pilot_reporting import (
    build_pilot_dossier,
    evaluate_pilot_run,
    run_post_submit_closed_loop,
)
...
def _seed(tmp_path):
    ...
    alert_file.write_text(
        json.dumps({"alert_type": "pilot_blocking", "severity": "critical"}) + "\n"
    )
...
def test_post_submit_closed_loop_creates_actions(tmp_path):
    state_file, gov_file, approval_file, alert_file, sample_file, funding_file = _seed(
        tmp_path
    )
    result = run_post_submit_closed_loop(
