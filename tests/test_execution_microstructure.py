...
def make_snapshot(
    close=100.0, bid=99.9, ask=100.1, mark_price=100.0, funding_rate=None, stale=False
):
...
def test_depth_impact_increases_fill_price():
    core = make_core(
        ExecutionConfig(
            ...
            simulated_depth_notional=100.0,
            impact_exponent=1.0,
        )
    )
...
def test_probabilistic_limit_fill_ratio_reduces_fill_quantity():
    core = make_core(
        ExecutionConfig(
            ...
            max_fill_ratio_per_bar=1.0,
            maker_fill_probability=0.25,
            queue_priority_model="probabilistic",
        )
    )
    order = core.create_order(
        OrderSide.BUY, quantity=10.0, order_type=OrderType.LIMIT, price=100.0
    )
    fill = core.try_fill_order(order, make_snapshot(close=100.0))[0]
