...
from btc_contract_backtest.config.models import ContractSpec, LeverageBracket, LiveRiskConfig, RiskConfig
...
def _policy(tmp_path, contract):
    ...
    return GovernancePolicy(RiskConfig(), LiveRiskConfig(), TradingMode.GUARDED_LIVE, contract=contract), approvals, alerts, audit


def test_cancel_replace_moves_record_into_pending_states(tmp_path):
    policy, approvals, alerts, audit = _policy(tmp_path, ContractSpec(symbol="BTC/USDT", leverage=3, lot_size=0.001, leverage_brackets=[LeverageBracket(notional_cap=1000.0, initial_leverage=20)]))
...
def test_cancel_replace_blocks_illegal_reduce_only_amend(tmp_path):
    policy, approvals, alerts, audit = _policy(tmp_path, ContractSpec(symbol="BTC/USDT", leverage=3, lot_size=0.001, leverage_brackets=[LeverageBracket(notional_cap=1000.0, initial_leverage=20)]))
    executor = GuardedLiveExecutor(CancelReplaceAdapter(), policy, approvals, alerts, audit)

    order = Order(order_id="o1", symbol="BTC/USDT", side=OrderSide.BUY, order_type=OrderType.MARKET, quantity=1.0, client_order_id="c1", reduce_only=True)
    record = canonical_record_from_order(order, submission_mode="governed_live")
    record = apply_local_submit(record, timestamp="2026-01-01T00:00:00+00:00")
    ...
    assert result["reason"] == "reduce_only_direction_invalid"
