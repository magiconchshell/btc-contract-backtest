...
def test_governance_blocks_stale_market():
    ...
    decision = policy.evaluate(symbol="BTC/USDT", notional=100, signal=1, stale=True, reconcile_ok=True, watchdog_halted=False)
...
def test_governance_requires_approval_when_mode_is_approval_required(tmp_path):
    policy = GovernancePolicy(RiskConfig(), LiveRiskConfig(), TradingMode.APPROVAL_REQUIRED)
    ...
    result = executor.submit_intended_order(symbol="BTC/USDT", signal=1, quantity=1.0, notional=100.0, stale=False, reconcile_ok=True, watchdog_halted=False)
