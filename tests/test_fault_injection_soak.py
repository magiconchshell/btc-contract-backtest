...
from btc_contract_backtest.live.fault_injection import EventSequenceMonitor, ResidualRiskInspector, SoakHarness
...
class PostCrashLookupAdapter:
    ...
    def fetch_open_orders_by_client_order_id(self, client_order_id):
        matches = [row for row in self.remote_orders if row.get("clientOrderId") == client_order_id]
...
def _executor(tmp_path, adapter, *, ledger_name="submit_ledger.json"):
    ...
    return GuardedLiveExecutor(adapter, policy, approvals, alerts, audit, submit_ledger=ledger), ledger
...
def test_recovery_orchestrator_converges_unknown_submit_after_restart(tmp_path):
    ...
    report = RecoveryOrchestrator(adapter, SubmitLedger(str(ledger_path))).recover(
        local_orders=[{"order_id": "o-1", "client_order_id": "c-1", "state": "new"}],
        local_position={},
        events=[
            {
                "sequence": 1,
