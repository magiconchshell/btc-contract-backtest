...
def main():
    version = 'v3'
    strategy_path = ROOT / 'strategies' / 'v3_low_freq_trend_follow.py'
    strategy = load_strategy(strategy_path)

    contract = ContractSpec(symbol='BTC/USDT', leverage=2)
    account = AccountConfig(initial_capital=100.0)
    risk = RiskConfig(max_position_notional_pct=0.35, maintenance_margin_ratio=0.005)
    ...
    (ROOT / 'reports' / 'v3_result.json').write_text(json.dumps(payload, indent=2, ensure_ascii=False))
    (ROOT / 'reports' / 'v3_summary.md').write_text(
        f"# v3 summary\n\n"
